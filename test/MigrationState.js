...
const getLegacyDeposition = async (depositor) => this.sfc.delegations.call(depositor);
...
const createLegacyDelegation = async(depositor, stakerID, amount, epoch) => {
  await this.sfc.createLegacyDelegation(stakerID, { from: depositor, value: amount});
...
contract('SFC', async ([firstStaker, secondStaker, firstDepositor, secondDepositor, thirdDepositor, fourthDepositor, fifthDepositor]) => {
  describe ('Delegation migration tests', async () => {
    ...
    beforeEach(async () => {
      this.firstEpoch = 0;
      this.sfc = await LegacyStaker.new(this.firstEpoch);
    ...
    it('should auto migrate legacy deposition to new model', async () => {
      ...
      const firstStakerID = await this.sfc.getStakerID(firstStaker);
      ...
      expect(await this.sfc.delegationsTotalAmount.call()).to.be.bignumber.equal(delegationAmount.mul(new BN('5')));
      expect(await this.sfc.delegationsNum.call()).to.be.bignumber.equal(new BN('5'));
      ...
      await this.sfc.increaseDelegation(firstStakerID, { from: firstDepositor, value: delegationAmount});
      // migrate second delegation (claimDelegationRewards)
      await this.sfc.makeEpochSnapshots(5);
      await this.sfc.claimDelegationRewards(currentEpoch, firstStakerID, { from: secondDepositor });
      ...
      const wrID = new BN('0');
      await this.sfc.discardDelegationRewards(firstStakerID, { from: fourthDepositor });
      await this.sfc.prepareToWithdrawDelegationPartial(wrID, firstStakerID, delegationAmount.div(new BN('2')), { from: fourthDepositor });
      // migrate fifth delegation (withdrawDelegation)
      await this.sfc.discardDelegationRewards(firstStakerID, { from: fifthDepositor });
      await this.sfc.prepareToWithdrawLegacyDelegation({ from: fifthDepositor });
      time.increase(86400 * 7);
      await this.sfc.makeEpochSnapshots(5);
      await this.sfc.makeEpochSnapshots(5);
      await this.sfc.makeEpochSnapshots(5);
      await this.sfc.withdrawDelegation(firstStakerID, { from: fifthDepositor });
    ...
    it('should manually migrate legacy deposition to new model', async () => {
      ...
      await this.sfc._syncDelegator(firstDepositor, firstStakerID);
      // check removed legacy delegations
      expect((await getLegacyDeposition(firstDepositor)).amount).to.be.bignumber.equal(new BN('0'));
      // check delegation in new model
      expect((await getDeposition(firstDepositor, firstStakerID)).toStakerID).to.be.bignumber.equal(firstStakerID);
    ...
    it('should not call calcDelegationRewards while delegation is in the legacy model', async () => {
      ...
      await this.sfc.makeEpochSnapshots(5);
      await this.sfc.makeEpochSnapshots(5);
      await expectRevert(this.sfc.calcDelegationRewards(firstDepositor, firstStakerID, new BN('0'), currentEpoch), "old version delegation, please update");
      // migrate delegation (_syncDelegator)
      await this.sfc._syncDelegator(firstDepositor, firstStakerID);
      // call calcDelegationRewards
      const rewards = await this.sfc.calcDelegationRewards(firstDepositor, firstStakerID, new BN('0'), currentEpoch);
